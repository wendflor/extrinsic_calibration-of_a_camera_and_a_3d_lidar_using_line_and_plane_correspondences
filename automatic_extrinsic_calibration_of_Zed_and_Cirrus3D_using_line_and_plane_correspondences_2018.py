...
from crop_point_cloud_on_target import crop_point_cloud
...
def init_estimate_t_one_pose(camera_coordinate_plane_equation, camera_coordinate_edges_equation,
    ...
    # create linear system, equation Matrix_left * t= Vector_right 
...
def cost_function_one_pose_keep_R_orthogonal_LM(
    x,
    camera_coordinate_plane_equation,
    camera_coordinate_edges_equation,
    lidar_plane_points,
    ...
    translation_vector = x[3:6].reshape(3, 1)

    # Normal vector and d of plane in camera coordinate
    n_c = np.reshape(camera_coordinate_plane_equation[:3], (-1, 1))
    d_c = camera_coordinate_plane_equation[3]

    # Points on calibration target edges in camera coordinate
    p1_c = np.reshape(camera_coordinate_edges_equation['left_lower_edge_equation'][0], (-1, 1))
    p2_c = np.reshape(camera_coordinate_edges_equation['left_upper_edge_equation'][0], (-1, 1))
    p3_c = np.reshape(camera_coordinate_edges_equation['right_upper_edge_equation'][0], (-1, 1))
    p4_c = np.reshape(camera_coordinate_edges_equation['right_lower_edge_equation'][0], (-1, 1))

    # Directions of calibration target edges in camera coordinate
    l1_c = np.reshape(camera_coordinate_edges_equation['left_lower_edge_equation'][1], (-1, 1))
    l2_c = np.reshape(camera_coordinate_edges_equation['left_upper_edge_equation'][1], (-1, 1))
    l3_c = np.reshape(camera_coordinate_edges_equation['right_upper_edge_equation'][1], (-1, 1))
    l4_c = np.reshape(camera_coordinate_edges_equation['right_lower_edge_equation'][1], (-1, 1))
    ...
    # Convert LiDAR points to proper format
    lidar_plane_points = lidar_plane_points.T
    lidar_l1_points = lidar_edges_points['left_lower_points'].T
    lidar_l2_points = lidar_edges_points['left_upper_points'].T
    lidar_l3_points = lidar_edges_points['right_upper_points'].T
    lidar_l4_points = lidar_edges_points['right_lower_points'].T

    # Compute residuals for points on the calibration target
    residuals_target = (n_c.T @ (rotation_matrix @ lidar_plane_points + translation_vector) + d_c).flatten()

    # Compute residuals for points on each edge
    residuals_edge_1 = (matrix_A_1 @ (rotation_matrix @ lidar_l1_points - p1_c + translation_vector)).flatten()
    residuals_edge_2 = (matrix_A_2 @ (rotation_matrix @ lidar_l2_points - p2_c + translation_vector)).flatten()
    residuals_edge_3 = (matrix_A_3 @ (rotation_matrix @ lidar_l3_points - p3_c + translation_vector)).flatten()
    residuals_edge_4 = (matrix_A_4 @ (rotation_matrix @ lidar_l4_points - p4_c + translation_vector)).flatten()
    ...
    total_residuals = np.concatenate([
        ...
        residuals_edge_3,
        residuals_edge_4
    ]).ravel()  # Flatten to a 1D array

    return total_residuals

def cost_function_one_pose_not_keep_R_orthognonal_LM(
    x,
    camera_coordinate_plane_equation,
    camera_coordinate_edges_equation,
    lidar_plane_points,
    ...
    rotation_matrix = x[0:9].reshape(3, 3)
    translation_vector = x[9:12].reshape(3, 1)
    ...
    # Compute residuals for points on each edge
    residuals_edge_1 = matrix_A_1 @ (rotation_matrix @ lidar_l1_points - p1_c + translation_vector)
    residuals_edge_1 = residuals_edge_1.flatten()

    residuals_edge_2 = matrix_A_2 @ (rotation_matrix @ lidar_l2_points - p2_c + translation_vector)
    residuals_edge_2 = residuals_edge_2.flatten()

    residuals_edge_3 = matrix_A_3 @ (rotation_matrix @ lidar_l3_points - p3_c + translation_vector)
    residuals_edge_3 = residuals_edge_3.flatten()

    residuals_edge_4 = matrix_A_4 @ (rotation_matrix @ lidar_l4_points - p4_c + translation_vector)
    residuals_edge_4 = residuals_edge_4.flatten()
...
def automatic_extrinsic_calibration_of_a_camera_and_a_3D_lidar_using_line_and_plane_correspondences(
    ...
    if optimization_mode == 'lm':
        if keep_rotation_matrix_orthogonal == True: 
            ...
            # Directly pass the function and additional arguments
            result = least_squares(
                fun=cost_function_one_pose_keep_R_orthogonal_LM, 
                x0=x0, 
                verbose=2,
                method='lm',
                args=(camera_coordinate_plane_equation, camera_coordinate_edges_equation, lidar_points_on_plane, lidar_points_on_edges)
            )
        else:
            ...
            # Directly pass the function and additional arguments
            result = least_squares(
                fun=cost_function_one_pose_not_keep_R_orthognonal_LM,
                x0=x0,
                verbose=2,
                method='lm',
                args=(camera_coordinate_plane_equation, camera_coordinate_edges_equation, lidar_points_on_plane, lidar_points_on_edges)
        ...
        result = least_squares(fun=fun, x0=x0, verbose=2, method='trf')
...
def lidar_points_in_image(
    ...
    plt.scatter(points_in_image[:, 0].tolist(), points_in_image[:, 1].tolist(), s=1, alpha=0.2)
    ax = plt.gca()
...
def automatic_extrinsic_calibration_of_a_camera_and_a_3D_lidar_using_line_and_plane_correspondences_2018(
    ...
    n = len(point_cloud_target)
    downsample_factor = 0.2  # Keep 20% of the points
    indices = np.random.choice(n, size=int(n * downsample_factor), replace=False)
    point_cloud_target = point_cloud_target[indices]
    '''
    ...
    if save_path is not None:
        ...
        filename = os.path.basename(img_rgb_path)  # This will give 'zed_calib_0000.png'

        # Split the filename to extract the number
        number = filename.split('_')[2].split('.')[0]

        dt_string = f"{number}_{dt_string}"
...
#############################  PARAMETERS  ######################################################################################## 
    # Define input parameters for extrinisic calibration
    number = '0000' # Number of image and point cloud calibration pair for a image in filepath: 'input_data/zed_calib/zed_calib_{number}.png' and point cloud in filepath:'input_data/Visionerf_calib/visionerf_calib_{number}.bin'
    num_row=6 # Amount of rows of checkerboard
    num_col=8 # Amount of columns of checkerboard
    square=18 # checker size (edge dimension in mm)
    display=False # Show visualization (True), else False
    save_path='output_data/' # Output filepath
    max_depth=530 # Filter value for point cloud w.r.t. z-Value (for 0000 --> 530, others ---> 455)

#############################  EXTRINSIC CALIBRATION  #############################################################################
    # Filter point cloud w.r.t. Z-Value
    raw_point_cloud_file_path=f'input_data/Visionerf_calib/visionerf_calib_{number}.bin'
    crop_point_cloud(raw_point_cloud_file_path, max_depth=max_depth)
    ...
    all_output = automatic_extrinsic_calibration_of_a_camera_and_a_3D_lidar_using_line_and_plane_correspondences_2018(
        img_rgb_path=f'input_data/zed_calib/zed_calib_{number}.png',
        point_cloud_on_target_path=f'input_data/Visionerf_calib/point_cloud_on_target_{number}.npy',
        point_cloud_whole_scene_path=f'input_data/Visionerf_calib/point_cloud_whole_scene_{number}.npy',
    ...
    # Results of extrinsic calibration 
    init_r = all_output['initial_r']
    init_t = all_output['initial_t']
    r = all_output['r'] # final rotation matrix
    t = all_output['t'] # final translation vector
